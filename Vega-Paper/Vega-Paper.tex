...
The inference rules for the equational deductive system are Substitution, Leibniz, and Transitivity of equality.
\[
...
An example of a proof step from the proof of theorem (\ref{E:expansionAlways}) Expansion of $\Always$ in Section 3.4 is
...
For example, here is a two-step proof of (\ref{E:idemUntil}) Idempotency of $\Until$, $p \Until p \equiv p$ in Section (3.2).
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> \>   $p\equiv p$\\[\lgap]
  \> which is (3.5) Reflexivity of $\equiv$
\end{tabbing}

Transitivity of equality is implicit in the proof.
Because $p \Until p\equiv p$ is equivalent to $p \lor (p \land \Next(p \Until p))\equiv p$,
and $p \lor (p \land \Next(p \Until p))\equiv p$ is equivalent to $p\equiv p$,
by transitivity $p \Until p\equiv p$ is equivalent to $p\equiv p$.\\
...
An example is a proof of (\ref{E:impEvent}) $p \impl \Event p$ in Section 3.3.
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> \>   $\Always (true\Until \Always p)$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:defEvent}) Definition of $\Event$}\\[\lgap]
  \> \>   $\Always\Event p$ \> \myqed
\end{tabbing}
}
...
\> \> $\Always p\land \Event q\impl p\Until q$
\end{tabbing}
which is (\ref{E:axiomUntilImpl}) $\Until$ implication. \hfill \myqed \\[\lgap]

$p\Until q$ is stronger than $p\Wait q$.
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $\impl$  \>  \Hint{(\ref{E:andUntilDist}) Distributivity of $\land$ over $\Until$ and (4.2) Monotonicity of $\lor$, $(p\impl q) \impl (p\lor r \impl q\lor r)$}\\[\lgap]
  \> \>   $(\Always p \land \Always q) \lor (p\land q)\Until (p\land r)$\\[\lgap]
  \> $=$ \> \Hint{(\ref{E:distAlwaysAnd}) Distributivity of $\Always$ over $\land$} \\[\lgap]
  \> \>   $\Always (p \land q) \lor (p\land q)\Until (p\land r)$\\[\lgap]
  \> $=$ \> \Hint{(\ref{E:defWait}) Definition of $\Wait$} \\[\lgap]
  \> \>   $(p \land q) \Wait (p \land r)$ \> \myqed
\end{tabbing}
...
[\emph{Conjectured theorem. Not yet proved}]
