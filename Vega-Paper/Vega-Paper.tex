...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:axiomAlwaysInduction}) with $q := false$}\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $\impl$  \>  \Hint{(\ref{E:axiomAlwaysInduction}) $\Always$ Induction}\\[\lgap]
  \> \>   $p \impl \Always p\lor p\Until q$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:alwaysImpWait}) Perpetuity and (3.82a) Transitivity}\\[\lgap]
...
%  \> $\impl$  \>  \Hint{(\ref{E:waitInsertion}) $\Wait$ Insertion and (3.82a) Transitivity}\\[\lgap]
%  \> \>   $\Always ( p \impl q \land \Next p) \land p  \impl  p \Wait \Always q$\\[\lgap]
%  \> $\impl$  \>  \Hint{(\ref{E:alwaysWaitImpl}) $\Wait\Always$ Implication and (3.82a) Transitivity}\\[\lgap]
