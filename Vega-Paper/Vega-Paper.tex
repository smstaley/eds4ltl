...
and the $\Always\Until$ axiom is required to 
...
The $\Until$ implication and $\Always \Until$ implication axioms are apparently unique to this system.
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
\> \> $\Always p$\\[\lgap]
\> $=$ \> \Hint{(\ref{E:untilInsertion}) Insertion, with $q:=\Always p$} \\[\lgap]
