...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
 ...
 \> $\impl$ \> \Hint{(3.76b) Strengthening, $p\land q \impl p$, and (3.82a) Transitivity, $(p\impl q) \land (q\impl r) \impl (p\impl r)$} \\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $\impl$  \>  \Hint{(\ref{E:distAlwaysImp}) Monotonicity of $\Always$ with $p, q := ( \Always r \land \neg \Always p \land \neg \Always q), (p \land q) $}\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$ \> \Hint{(3.43b) Absorption $p \lor (p \land q) \equiv p$ and (3.24) Symmetry of $\lor$, $p\lor q \equiv q\lor p$} \\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
  \> $=$  \>  \Hint{Lemma C: $(\Always p \lor \Always q) \impl \Next (\Always p \lor \Always q)$ is a theorem.}\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
 ...
 \> $\impl$  \>  \Hintfirst{Lemma D: $\Always (\Always p \lor \Always q) \impl \Always (\Always p \lor \Always q \lor (p \land q))$ }\\[\lgap]
 ...
 \>     \>  \Hintlast{with $p, q, r := (\Always p \lor \Always q), \Always (\Always p \lor \Always q), \Always (\Always p \lor \Always q \lor (p \land q))$}\\[\lgap]
  \> \>   $\Always p \lor \Always q \impl \Always (\Always p \lor \Always q \lor (p \land q))$\\[\lgap]
   \> $=$  \>  \Hint{Lemma E: $\Always ((p \lor \Always q) \land (\Always p \lor q)) \equiv \Always (\Always p \lor \Always q \lor (p \land q))$}\\[\lgap]
