...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $\impl$  \>  \Hint{(\ref{E:CompInductRule}) Computational induction rule}\\[\lgap]
  \> \>   $p \impl \Always \neg q$\\[\lgap]
...
The nine axioms correspond to (\ref{E:defEvent}) Definition of $\Event$, (\ref{E:defAlways}) Definition of $\Always$, (\ref{E:distNextOr}) Distributivity of $\Next$ over $\lor$, (\ref{E:linearity}) Linearity, (\ref{E:expansionUntil}) Expansion of $\Until$, (\ref{E:nextTruth}) Truth of $\Next$, (\ref{E:CompInductRule}) and (\ref{E:CompInductRule2Em}) Computational induction rules, and (\ref{E:alwaysImpNexts}) Monotonicity of $\Next$.
