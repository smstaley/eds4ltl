...
with itself and with equivaloes
...
\textbf{Axiom, Distributivity of $\Always$ over $\Rightarrow$:}\quad \Always (p \Rightarrow q) \Rightarrow (\Always p \Rightarrow \Always q)
\end{equation}
...
\hspace{\mymathindent} \= $= \;$ \= \kill
  ...
  \> $=$  \>  \Hint{(3.46) Distributivity of }\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \kill
  ...
  \> $=$  \>  \Hint{(3.80) Mutual implication}\\[\lgap]
  \> \>   $\Always ((p \Rightarrow q) \land (q \Rightarrow p))$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:distAlwaysAnd}) Distributivity of $\Always$ over $\land$}\\[\lgap]
  \> \>   $\Always (p \Rightarrow q) \land \Always (q \Rightarrow p)$\\[\lgap]
  \> $\Rightarrow$  \>  \Hint{(\ref{E:distAlwaysImp}) Distributivity of $\Always$ over $\Rightarrow$, and (4.3) Monotonicity of $\land$}\\[\lgap]
  \> \>   $(\Always p \Rightarrow \Always q) \land (\Always q \Rightarrow \Always p)$\\[\lgap]
  \> $=$  \>  \Hint{(3.80) Mutual implication}\\[\lgap]
  \> \>   $\Always p \equiv \Always q$\\[\lgap]
\end{tabbing}
\myqed\\[\lgap]
...
\Always (p \Rightarrow \Next p) \Rightarrow (p \Rightarrow \Always p)
\end{equation}



\begin{equation}\label{E:alwaysImpNexts}
\Always (p \Rightarrow q) \Rightarrow (\Next p \Rightarrow \Next q)
\end{equation}
