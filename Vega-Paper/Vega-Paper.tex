...
From this axiom, subsequent theorems prove that the next operator distributes over all the binary operators, both propositional and temporal.\\
...
The other direction is given by Axiom (\ref{E:distAlwaysEventOrAx}).
...
The other direction is given by Axiom (\ref{E:distEventAlwaysAndAx}).
...
\Event p \equiv \lnot(\lnot p \Wait false)
\end{equation}

\emph{Proof:}
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \kill
\> \> $\lnot(\lnot p \Wait false)$\\[\lgap]
...
(p \Wait q) \land \Always\lnot q \impl \Always p
\end{equation}

\emph{Proof:}
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \kill
\> \> $(p \Wait q) \land \Always\lnot q$\\[\lgap]
\> $\impl$ \> \Hint{(\ref{E:waitEntailment})} \\[\lgap]
\> \> $(\Always p \lor \Event q) \land \Always\lnot q$\\[\lgap]
\> $=$ \> \Hint{(3.46) Distributivity of $\land$ over $\lor$} \\[\lgap]
\> \> $(\Always p \land \Always\lnot q) \lor (\Event q \land \Always\lnot q)$\\[\lgap]
\> $=$ \> \Hint{(\ref{E:dualEvent})} \\[\lgap]
\> \> $(\Always p \land \Always\lnot q) \lor (\Event q \land \lnot\Event q)$\\[\lgap]
\> $=$ \> \Hint{(3.42) Contradiction} \\[\lgap]
\> \> $(\Always p \land \Always\lnot q) \lor false$\\[\lgap]
\> $=$ \> \Hint{(3.30) Identity of $\lor$} \\[\lgap]
\> \> $\Always p \land \Always\lnot q$\\[\lgap]
\> $\impl$ \> \Hint{(3.76b) Strengthening} \\[\lgap]
\> \> $\Always p$\\[\lgap]
\end{tabbing}
\myqed\\[\lgap]
...
(p \Wait q) \land \lnot\Always p \impl \Event q
\end{equation}

\emph{Proof:}
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \kill
\> \> $(p \Wait q) \land \lnot\Always p$\\[\lgap]
\> $\impl$ \> \Hint{(\ref{E:waitEntailment})} \\[\lgap]
\> \> $(\Always p \lor \Event q) \land \lnot\Always p$\\[\lgap]
\> $=$ \> \Hint{(3.46) Distributivity of $\land$ over $\lor$} \\[\lgap]
\> \> $(\Always p \land \lnot\Always p) \lor (\Event q \land \lnot\Always p)$\\[\lgap]
\> $=$ \> \Hint{(3.42) Contradiction} \\[\lgap]
\> \> $false \lor (\Event q \land \lnot\Always p)$\\[\lgap]
\> $=$ \> \Hint{(3.30) Identity of $\lor$} \\[\lgap]
\> \> $\Event q \land \lnot\Always p$\\[\lgap]
\> $\impl$ \> \Hint{(3.76b) Strengthening} \\[\lgap]
\> \> $\Event q$\\[\lgap]
\end{tabbing}
\myqed\\[\lgap]
