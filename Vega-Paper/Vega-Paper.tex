...
This paper extends the equational deductive system of Gries and Schneider to linear temporal logic (LTL), using only the same four inference rules.
The paper surveys the LTL literature and includes all the LTL theorems from the survey plus many new and interesting ones.
...
In (2.3b) Metatheorem Duality with $P$ the expression $\Next\neg p$ and $Q$ the expression $\neg\Next p$, both $P$ and $Q$ are equivalent to their dual expressions, and so the application of the metatheorem is trivial.
...
Theorem (\ref{E:distNextAnd}) follows from (\ref{E:distNextOr}) by (2.3b) Metatheorem Duality,
...
Theorem (\ref{E:nextFalse}) follows from (\ref{E:nextTruth}) by (2.3b) Metatheorem Duality,
but the following proof uses (\ref{E:linearity}) Linearity instead of duality.
...
% begin sms
The next theorem (\ref{E:metaDeduction}) is a proof metatheorem
based on the temporal deduction theorem (2.1.6) of Kröger and Merz \cite{Kroger}.
To prove $\Always P \impl Q$, assume $P$ and prove $Q$. You cannot use textual substitution in $P$.
...
\end{tabbing}
And now, assume $p \equiv true$ and prove $q \Until r \impl (p \land q) \Until (p \land r)$
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $q \Until r \impl (p \land q) \Until (p \land r)$\\[\lgap]
  \> $=$  \>  \Hint{Assumed $p \equiv true$}\\[\lgap]
...
\end{tabbing}
And now, assume $p \equiv true$ and prove $\Event q  \impl p \Until q$
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$  \>  \Hint{(3.65) Shunting, $p\land q \impl r \equivs p \impl (q \impl r)$}\\[\lgap]
  \> \>   $\Always (p \impl q) \land p \Until r \impl q \Until r$\\[\lgap]
  \> $=$  \>  \Hint{Lemma: $\Always (p \impl q) \land p \Until r \impl q \Until r$}\\[\lgap]
  \> \>   $true$
\end{tabbing}
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:excludedMidc}) is a theorem}\\[\lgap]
  \> \>   $\Event p \lor \Event\neg p$\\[\lgap]
   \> $=$  \>  \Hint{(3.3) Identity of $\equiv$, $true \equiv p \equiv p$}\\[\lgap]
   \> \>   $\Event p \lor \Event\neg p \equiv true$\\[\lgap]
    \> $=$  \>  \Hint{(2.3b) Metatheorem Duality, $(P \equiv Q) \equiv (P_D \equiv Q_D)$}\\[\lgap]
    \> \>   $\Always p \land \Always \neg p \equiv false$ \quad \myqed
\end{tabbing}
% end sms
% begin sms
...
\hspace{\mymathindent} \= $= \;$ \= $= \;$ \= \kill
  ...
  \> $\impl$  \>  \Hintfirst{(\ref{E:impAlways}) Strengthening of $\Always$ and (3.82a) Transitivity,}\\[\lgap]
  \> \>       \Hintlast{$(p\impl q) \land (q\impl r) \impl (p\impl r)$ and (4.3) Monotonicity of $\land$}\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$  \>  \Hint{(\ref{E:distAlwaysAnd}) Distributivity of $\Always$ over $\land$}\\[\lgap]
   ...
   \> $\impl$  \>  \Hint{(\ref{E:rightMonoUntil}) Right monotonicity of $\Until$ with $p, q, r := r, q \Until s, q$ }\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$  \>  \Hint{(\ref{E:distAlwaysAnd}) Distributivity of $\Always$ over $\land$}\\[\lgap]
   ...
   \> $\impl$  \>  \Hint{(\ref{E:rightMonoUntil}) Right monotonicity of $\Until$ and (4.3) Monotonicity of $\land$}\\[\lgap]
    \> \>   $\Always (p \impl r) \land p \Until s$\\[\lgap]
    \> $\impl$  \>  \Hint{(\ref{E:leftMonoUntil}) Left monotonicity of $\Until$}\\[\lgap]
...
\subsection{Always, continued}\label{section-always-continued}
% begin sms
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:CompInductRule}) Computational induction rule
  with $p,q := p \Until \Always q, p \Until q$ is a theorem}\\[\lgap]
  \> \>   $\Always (p \Until \Always q \impl (p \Until q \land \Next (p \Until \Always q))) \impl
  (p \Until \Always q \impl \Always (p \Until q))$\\[\lgap]
   \> $=$  \>  \Hintfirst{(3.63.1) Distributivity of $\impl$ over $\land$, $p\impl q\land r\equivs (p\impl q)\land (p\impl r)$}\\[\lgap]
   \> \>   \Hintlast{with $p,q,r := p \Until \Always q, p \Until q, \Next (p \Until \Always q)$}\\[\lgap]
   \> \>   $\Always ((p \Until \Always q \impl p \Until q) \land (p \Until \Always q \impl \Next (p \Until \Always q))) \impl
   (p \Until \Always q \impl \Always (p \Until q))$\\[\lgap]
   \> $=$  \>  \Hint{(\ref{E:distAlwaysAnd}) Distributivity of $\Always$ over $\land$}\\[\lgap]
  \> \>   $\Always (p \Until \Always q \impl p \Until q) \land \Always (p \Until \Always q \impl \Next (p \Until \Always q)) \impl
   (p \Until \Always q \impl \Always (p \Until q))$\\[\lgap]
  \> $=$  \>  \Hint{Lemma A:  $\Always (p \Until \Always q \impl p \Until q)$ is a theorem}\\[\lgap]
  \> \>   $true \land \Always (p \Until \Always q \impl \Next (p \Until \Always q)) \impl
   (p \Until \Always q \impl \Always (p \Until q))$\\[\lgap]
   \> $=$  \>  \Hint{Lemma B:  $\Always (p \Until \Always q \impl \Next (p \Until \Always q))$ is a theorem}\\[\lgap]
  \> \>   $true \land true \impl (p \Until \Always q \impl \Always (p \Until q))$\\[\lgap]
    \> $=$  \>  \Hintfirst{(3.38) Idempotency of $\land$, $p\land p \equiv p$}\\[\lgap]
      \>  \>  \Hintlast{and (3.73) Left identity of $\impl$,  $true\impl p \equiv p$}\\[\lgap]
  \> \>   $p \Until \Always q \impl \Always (p \Until q)$ \\[\lgap]
\end{tabbing}
% need lemmas here
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
  \> $=$ \> \Hint{(\ref{E:impAlways}) Strengthening of $\Always$ with $p:=q$ is a theorem} \\[\lgap]
  \> \>   $\Always q \impl q$\\[\lgap]
   ...
   \> $=$ \> \Hint{(\ref{E:metatheorem}) Metatheorem with the above theorem}\\[\lgap]
   \> \>   $\Always (p \Until \Always q \impl p \Until q)$\\[\lgap]
\end{tabbing}
...
First prove $p \Until \Always q \impl \Next (p \Until \Always q)$ and
then apply (\ref{E:metatheorem}) Metatheorem to finish the proof.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
...
\> $=$  \>  \Hint{(\ref{E:distNextUntil}) Distributivity of $\Next$ over $\Until$}\\[\lgap]
\> \> $ \Next (p \Until \Always q)$\\[\lgap]
\end{tabbing}
This proves $p \Until \Always q \impl \Next (p \Until \Always q)$ is a theorem. And
therefore, by (\ref{E:metatheorem}) Metatheorem, the following is a theorem.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
\> \>   $\Always (p \Until \Always q \impl \Next (p \Until \Always q))$ \quad \myqed
\end{tabbing}
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:AEexcludedMid2}) is a theorem}\\[\lgap]
  \> \>   $\Always \Event p \lor \Always \Event\neg p$\\[\lgap]
   \> $=$  \>  \Hint{(3.3) Identity of $\equiv$, $true \equiv p \equiv p$}\\[\lgap]
   \> \>   $\Always \Event p \lor \Always \Event\neg p \equiv true$\\[\lgap]
    \> $=$  \>  \Hint{(2.3b) Metatheorem Duality, $(P \equiv Q) \equiv (P_D \equiv Q_D)$}\\[\lgap]
    \> \>   $\Event \Always p \land \Event \Always \neg p \equivs false$ \quad \myqed
\end{tabbing}
% end sms
% insert frame laws here
...
The following theorem is Exercise 14.7 in \cite{Ben}.
...
\emph{Proof of Lemma C}: The proof is by (4.7.1) Truth implication.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
\> \>   $true$\\[\lgap]
...
\emph{Proof of Lemma D}: The proof is by (4.7.1) Truth implication.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
   \> $=$  \>  \Hint{(3.71) Reflexivity of $\impl$, $p\impl p$}\\[\lgap]
...
\emph{Proof of Lemma G}: The proof is by (4.7.1) Truth implication.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
 ...
 \> $=$  \>  \Hint{(\ref{E:distAlwaysAnd}) Distributivity of $\Always$ over $\land$}\\[\lgap]
  ...
  \> $=$  \>  \Hint{(\ref{E:excludedMid}) Temporal excluded middle and (3.39) Identity of $\land$, $p\land true\equiv p$ }\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $\foll$  \>  \Hint{(\ref{E:untilImplWait}) $p \Until q \impl p \Wait q$ and (4.2) Monotonicity of $\lor$}\\[\lgap]
...
The axiomatization of LTL in Kröger and Merz consists of 8 Axioms and 3 inference rules. The
8 axioms of the Kröger and Merz LTL proof system with the corresponding equivalent theorems of this work
are shown in Figure \ref{kroger-axioms}.\\
% add table here sms
\begin{figure}[t]
\centering
\renewcommand\arraystretch{1.2}
\begin{tabular}{ l l }
  ...
  &
  Also assumed in this work. \\
  \midrule
  (ltl1) $\Next \neg p \equiv \neg \Next p$
  &
  This is an axiom (\ref{E:selfDual}) Self-dual.\\
  \midrule
  (ltl2, ltl2') $\Next (p \impl q)  \equiv ( \Next p \impl \Next q)$
  &
  This is an axiom (\ref{E:distNextImp}) Distributivity of $\Next$ over $\impl$.\\
   \midrule
  (ltl3, ltl3') $\Always p \equiv p \land \Next\Always p$
  &
  This is a theorem
 (\ref{E:expansionAlways}) Expansion of $\Always$.\\
   \midrule
  (Tb13) $p \Until q \equiv q \lor (p \land \Next (p \Until q))$
  &
  This is an axiom (\ref{E:expansionUntil}) Expansion of $\Until$.\\
   \midrule
  (unt2) $p \Until q \impl \Event q$
  &
  This is a theorem (\ref{E:eventuality}) Eventuality.\\
   \midrule
 (Tb14) $p \Wait q \equiv q \lor (p \land \Next (p \Wait q))$
  &
  This is a theorem (\ref{E:expansionWait}) Expansion of $\Wait$.\\
  \midrule
  (unl2) $\Always p \impl p \Wait q$
  &
  This is a theorem (\ref{E:alwaysImpWait}) Perpetuity.\\
  \bottomrule
\end{tabular}
\renewcommand\arraystretch{1}
\caption{The eight axioms of the Kröger and Merz LTL proof system with the corresponding equivalent theorems of this work.
\label{kroger-axioms}}
\end{figure}
% end of table sms
