...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
	...
	\> $=$  \>  \Hintfirst{(8.19) Interchange of dummies $(\star x\dr R\rb (\star y\dr Q\rb P)) = (\star y\dr Q\rb (\star x\dr R\rb P))$}\\[\lgap]
...
The following three axioms describe how the \textit{until} operator distributes over conjunction and disjunction.
The fourth property, right distributivity of \$Until\ over $\land$ is theorem (\ref{E:untilAndEquiv}) in Section 3.5.
...
The absorption axiom is required to prove the absorption properties of $\Until$ and $\Wait$,
and the $\Until$ implication axiom is required to derive an expression for $\Until$ in terms of $\Wait$.
The distributivity axioms are required to prove the theorems where $\Always\Event$ distributes over $\lor$ and $\Event\Always$ distributes over $\land$ with equality.
The absorption, $\Until$ implication, and $\Always \Until$ implication axioms are apparently unique to this system.
...
Two axioms characterize the \textit{wait} operator, the definition of $\Wait$ in terms of $\Until$ and the right distributivity of
$\Wait$ over $\land$.
...
The next three distributive theorems for the \textit{wait} operator correspond to, and are proved from,
the distributive axioms for the \textit{until} operator
(\ref{E:untilOrEquiv}), (\ref{E:untilOrImp}), and (\ref{E:untilAndImp}).
The fourth distributive theorem for the right distributivity of $\Until$ over $\land$ corresponds to, and is proved from, the distributive
axiom for the \textit{wait} operator (\ref{E:rightWaitAndDist}).
...
The next pair of theorems show how the unary temporal operators absorb into the binary temporal operators.
...
\emph{Proof:} By (\ref{E:metaAlways}) Metatheorem and (\ref{E:waitToOr}), $\Always (p\Wait q\impl p\lor q)$ is a theorem.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
...
The following theorem for the \textit{wait} operator corresponds to axiom (\ref{E:axAbsorpUntil}) Absorption for the \textit{until} operator.
