...
\textbf{Axiom, Induction:}\quad \Always (p \Rightarrow \Next p) \Rightarrow (p \Rightarrow \Always p)
\end{equation}
...
\hspace{\mymathindent} \= $= \;$ \= \kill
  ...
  \> $=$  \>  \Hint{(3.57) Definition of Implication}\\[\lgap]
  \> \>   $\Always p \lor \Always q \Rightarrow \Always(p \lor q)$
\end{tabbing}
...
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \kill
  \> \>   $\Event\Always p \Rightarrow \Always\Event p$\\[\lgap]
  \> $=$  \>  \Hint{(3.59) Implication}\\[\lgap]
...
\emph{Proof of:} $\Event\Always\Event p \Rightarrow \Always\Event p$\\

\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \kill
  \> \>   $\Event\Always\Event p$\\[\lgap]
  \> $\Rightarrow$  \>  \Hint{(\ref{E:eventAlwaysImp})}\\[\lgap]
...
\opt{doc}{
\emph{Proof of:} $\Always\Event\Always p \Rightarrow \Event\Always p$
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \kill
