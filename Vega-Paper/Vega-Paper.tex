...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> \>   $\Always (p \Until \Always q \impl p \Until q) \land \Always (p \Until \Always q \impl \Next (p \Until \Always q)) \impl
   (p \Until \Always q \impl \Always (p \Until q))$\\[\lgap]
  \> $\impl$  \>  \Hint{Lemma A:  $\Always (p \Until \Always q \impl p \Until q)$ is a theorem}\\[\lgap]
  \> \>   $true \land \Always (p \Until \Always q \impl \Next (p \Until \Always q)) \impl
   (p \Until \Always q \impl \Always (p \Until q))$\\[\lgap]
   \> $\impl$  \>  \Hint{Lemma B:  $\Always (p \Until \Always q \impl \Next (p \Until \Always q))$ is a theorem}\\[\lgap]
...
\emph{Proof of Lemma B}: First prove $p \Until \Always q \impl \Next (p \Until \Always q)$ and
