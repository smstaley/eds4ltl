...
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $\Always (p => q) \land r \Until p$\\[\lgap]
  \> $\impl$  \>  \Hint{(\ref{E:andUntilDist}) Distributivity of $\land$ over $\Until$}\\[\lgap]
  \> \>   $((p \impl q) \land r) \Until ((p \impl q) \land p)$\\[\lgap]
  \> $=$  \>  \Hint{(3.59) Implication, $p \impl q \equiv \lnot p \lor q$}\\[\lgap]
  \> \>   $((p \impl q) \land r) \Until ((\lnot p \lor q) \land p)$\\[\lgap]
  \> $=$  \>  \Hint{(3.44a) Absorption, $p \land (\lnot p \lor q)$}\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$  \>  \Hint{(3.59) Implication, $p \impl q \equiv \lnot p \lor q$}\\[\lgap]
  \> \>   $((\lnot p \lor q) \land p) \Wait ((p \impl q) \land r)$\\[\lgap]
  \> $=$  \>  \Hint{(3.44a) Absorption, $p \land (\lnot p \lor q)$}\\[\lgap]
