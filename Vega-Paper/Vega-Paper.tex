...
\begin{equation}\label{E:untilImplEvent}
p \Until q\impl \Event q
\end{equation}

\emph{Proof:}
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $p \Until q\impl \Event q$\\[\lgap]
  \> $=$  \>  \Hint{(3.60) Implication, $p\impl q \equiv p\land q\equiv p$}\\[\lgap]
  \> \>   $p \Until q\land \Event q\equiv p\Until q$\\[\lgap]
\end{tabbing}
which is (\ref{E:absEventIntoUntil}) Absorption of $\Event$ into $\Until$. \hfill \myqed\\[\lgap]
...
The absorption of $\land$ into $\Always$ mirrors (\ref{E:absOrIntoEvent}) the absorption of $\lor$ into $\Event$,
while he absorption of $\lor$ into $\Always$ mirrors (\ref{E:absAndIntoEvent}) the absorption of $\land$ into $\Event$.
...
p \Until q \equiv p \Wait q\land \Event q
\end{equation}
...
\> \> $\Event q$ \> \myqed
\end{tabbing}
Absorption of $\Always$ into $\Wait$ corresponds to (\ref{E:absEventIntoUntil}) Absorption of $\Event$ into $\Until$.
