...
\subsubsection*{Axiomatic logic systems}
...
If $E[z:=true]$ and $E[z:=false]$ are theorems, then so is $E[z:=p]$.
This metatheorem does \textit{not} hold in LTL because the two cases, $z:=true$ and $z:=false$, only account for two out of an infinite number of possible sequences of T's and F's in $\sigma$.
...
\hspace{\mymathindent} \= (2.3)\; \= \myqedtab \= \kill
  ...
  \>       \> (a) \verb|valid(|$P$\verb|) |$\equiv$\verb| valid(|$\neg P_D$\verb|)|\\
  \>       \> (b) \verb|valid(|$P \equiv Q$\verb|) |$\equiv$\verb| valid(|$P_D \equiv Q_D$\verb|)|
\end{tabbing}
where \verb|valid(|$R$\verb|)| is a predicate that returns true iff $R$ is a valid expression.
...
In (2.3b) Metatheorem Duality $(P \equiv Q) \equiv (P_D \equiv Q_D)$ with $P: \Next\neg p$ and $Q: \neg\Next p$, both $P$ and $Q$ are equivalent to their dual expressions.
...
The proof of (\ref{E:nextFalse}) uses (3.8) Definition of $false$, $false\equiv \neg true$.
...
Theorem (\ref{E:eventuality}) shows that $p\Until q$ guarantees that $q$ will eventually be $true$.
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$  \>  \Hint{(\ref{E:absUntilIntoEvent}) Absorption of $\Until$ into $\Event$, with $p,q := true, p$}\\[\lgap]
  \> \>   $\Event p$ \quad \myqed
\end{tabbing}
}
...
It follows from (\ref{E:untilInduction2}) with $q:=false$.
...
The short proof of (\ref{E:untilAndImplUntilUntil}) illustrates the use of methatheorem (\ref{E:metatheorem}).
...
The proof of (\ref{E:distEventAlwaysOr}) illustrates the use of (\ref{E:metaEvent}) Metatheorem $\Event$.
...
Theorem (\ref{E:rightZeroWait}) shows that $true$ is the right zero of \textit{wait}, as it is for \textit{until}.
Theorem (\ref{E:leftIdentWait}) shows that $false$ is the left identity of \textit{wait}, as it is for \textit{until}.
...
This system has 252 theorems, 19 of which are axioms.
...
David Gries provided a prepublication manuscript of LADM, which we immediately adopted and have used ever since.
He has been a source of constant encouragement.
Ravi Mohan and Ray McIntyre contributed to the proofs.
