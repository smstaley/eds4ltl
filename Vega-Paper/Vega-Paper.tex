...
Gries [citation needed] shows that $\mathcal{E}$, also known  as a \textit{calculational} system [citation needed], has several advantages over traditional logic systems.
The primary advantage of $\mathcal{E}$ over $\mathcal{H}$ is that the equational system has only one proof rule, Leibniz.
Consequently, proofs of theorems are easier to understand and more intuitive to those schooled in that system.
\textit{blah, blah} $\dots$
...
In 1994, Gries and Schneider published \textit{A Logical Approach to Discrete Math} (LADM) \cite{LADM}, in which they extend the equational system $\mathcal{E}$ to a theory of sets, a theory of sequences, relations and functions, a theory of integers, recurrence relations, modern algebra, and a theory of graphs.
...
The only appearance of an equational proof of a temporal logic theorem appears to be a single example in \cite{Schn}, page XX, which otherwise uses a Hilbert-style system for temporal logic.
Schneider states in \cite{Schn}, page YY, ``\textit{blah, blah}\dots''.

To solve the above problem, this paper presents an equational deductive system for linear temporal logic.
...
The predicate calculus of the equational system unifies quantification between mathematics and logic with nine axioms that apply to Abelian monoids, denoted $\star$, in general.

For example, the standard way to write the sum of the squares of the first $n$ positive integers is
\[
\sum_{i=1}^{n}i^2
\]
where $\star$ is $+$ and $\upSigma$ is the quantified symbol for addition.
The equational notation for the same expression is
\[
(\upSigma i\dr 1\le i\le n \rb i^2)
\]
Similarly, the standard way to write that there exists a number between 10 and 20 inclusive that divides $n$ is
\[
\ext i(10\le i\le 20 \land \divides(i,n))
\]
where $\star$ is $\lor$, $\ext$ is the quantified symbol for disjunction, and $\divides$ is a predicate that is true when $i$ divides $n$.
The equational notation for the same expression is
\[
(\ext i\dr 10\le i\le 20 \rb \divides(i,n))
\]

All such quantifications have explicit scope for the dummy variable denoted by the outer parentheses.
Within the parentheses the quantification consists of three parts:
\begin{itemize}
\item the operator and dummy variable(s),
...
In the first example above, the operation is addition, the dummy variable is $i$, the range is $1\le i\le n$, and the body is $i^2$.
...
For example, in the following sequence $\Next 10\le x< 13$ holds at state $s_1$ because $\Next 10\le x< 13$ holds at state $s_2$.\\[\lllgap]
...
\end{tabbing}
The following figure illustrates the temporal segments for which $p$, $q$, and $r$ hold for \\$p\Until(q\Until r)$.\\[\lgap]
...
It is the basis of axiom (\ref{E:untilAssocImp}), Weak associativity of $\Until$, $p \Until (q \Until r) \impl (p \Until q) \Until r$ in Section 3.2.
...
\end{tabbing}
The following figure illustrates the temporal segments for which $p$ and $q$ hold for $p\Until \Always q$.\\[\lgap]
\begin{picture}(360,120)
\thicklines
...
while he absorption of $\Always$ into $\lor$ corresponds to (\ref{E:absEventIntoAnd}) the absorption of $\Event$ into $\land$.
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> which is (3.5) Reflexivity of $\equiv$, $p\equiv p$.
\end{tabbing}
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$  \>  \Hint{(3.60) Implication, $p\impl q \equivs p\land q \equivs p$}\\[\lgap]
  \> \>   $\Event(p \land q) \impl \Event p \land \Event q$\\[\lgap]
  \> which is (\ref{E:distEventAnd}) Distributivity of $\Event$ over $\land$. \quad \myqed
\end{tabbing}
...
\emph{Proof}: \opt{doc}{ The proof is by (4.12) Contrapositive.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$  \>  \Hint{(3.47b) De Morgan, $\neg (p \lor q) \equiv \neg p \land \neg q$}\\[\lgap]
  \> \>   $\neg (\Event\Always p\lor \Event\Always q)$ \quad \myqed
\end{tabbing}
}
\opt{student}{
Exercise for the student.\\
...
The table below summarizes the eight possibilities of $true$ and $false$ on the left hand side and right hand side of $\Until$ and $\Wait$.\\[\lllgap]
