...
\newcommand{\myqedtab}{\hspace{388.5pt}}              % For flush right qed symbol in tabbing environment
...
An example of a proof step from the proof of theorem (\ref{E:expansionAlways}) Expansion of $\Always$ from Section 3.4 is
...
The justification in the hint $X=Y$ comes from the textual substitution of $\neg p$ for $q$
...
This result is theorem (\ref{E:zeroUntil}) $p \Until true \equiv true$ proved in Section 3.2.
$true$ is the right zero of the \textit{until} operator.\\

\opt{doc}{
The \textit{until} operator $\Until$ is not associative as shown by the following sequence.\\
...
From the table, do you believe the \textit{until} operator $\Until$ is associative?
...
Yet another defining axiom for the \textit{always} operator that does not seem to appear in the temporal logic literature is
...
The following figure illustrates the temporal segments for which $p$ and $q$ hold for $p\Until \Always q$.

\begin{picture}(360,120)
\thicklines
\put(0,100)  {\line(1,0){360}}
\put(20,100) {\circle*{8}} \put(180,100) {\circle*{8}}
\put(18,86)  {$j$} \put(178,86)  {$k$}
\put(168,74) {$k\ge j$}
\put(180,52) {\line(1,0){180}}
\put(180,52) {\circle*{8}}
\put(240,58) {$(\sigma,m)\models q$}
\put(250,42) {$m\ge k$}
\put(20,20)  {\line(1,0){156}}
...
From Case 1 and Case 2, $p\Until q$ holds at $(\sigma,i)$ for $i\ge j$, which completes the demonstration.
...
\end{tabbing}
which is (\ref{E:nextTruth}) Truth of $\Next$. \hfill \myqed
}
...
%\> \> $p\Wait r\land \Event r\land q\Wait r\land \Event r$\\[\lgap]
%\> $=$ \> \Hint{(3.38) Idempotency of $\land$, $p \land p \equiv p$} \\[\lgap]
%\> \> $p\Wait r\land q\Wait r\land \Event r$\\[\lgap]
%\> $=$ \> \Hint{(\ref{E:rightWaitAndDist}) Axiom, Right distributivity of $\Wait$ over $\land$} \\[\lgap]
%\> \> $(p\land q)\Wait r\land \Event r$\\[\lgap]
%\> $=$ \> \Hint{(\ref{E:untilFromWait}) $\Until$ in terms of $\Wait$} \\[\lgap]
%\> \> $(p \land q) \Until r$ \> \myqed
%\end{tabbing}
...
The following four absorption theorems do not seem to appear in the temporal logic literature.
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $\Leftarrow$  \>  \Hint{(3.76a) Weakening the consequent, $p \impl p\lor q$}\\[\lgap]
  \> \>   $\Next p$ \> \myqed
\end{tabbing}
...
The $\Event\Always$ implication axiom states that $\Event\Always p$ ensures that $p$ will always eventually hold, but not the converse.
...
The $\Until$ implication axiom is required to derive an expression for $\Until$ in terms of $\Wait$,
and the $\Always\Until$ implication axiom is required to prove absorption of $\Until$ into $\Always$.
The $\Until$ implication and $\Always \Until$ implication theorems are apparently unique to this system.
\begin{equation}\label{E:eventAlwaysImp}
\textbf{Axiom, $\Event \Always$ implication:}\quad \Event\Always p \impl \Always\Event p
\end{equation}
...
\> $\impl$ \> \Hint{(\ref{E:axiomAlwaysUntilImpl}) $\Always \Until$ implication, with $q:=p$} \\[\lgap]
...
\end{tabbing}
which is (\ref{E:eventTrue}) Truth of $\Event$. \hfill \myqed \\
}
...
while he absorption of $\lor$ into $\Always$ corresponds to (\ref{E:absAndIntoEvent}) the absorption of $\land$ into $\Event$.
...
as in (\ref{E:absOrIntoEvent}) and (\ref{E:absOrIntoAlways}), the equivalence is just a statement about $p$ at the current time.
As with (\ref{E:absOrIntoEvent}) and (\ref{E:absAndIntoEvent}), the following two absorption theorems
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $p \lor \Always p$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:expansionAlways}) Expansion of $\Always$}\\[\lgap]
  \> \>   $p \lor (p \land \Next\Always p)$\\[\lgap]
  \> $=$  \>  \Hint{(3.43b) Absorption, $p\lor (p \land q) \equiv p$}\\[\lgap]
  \> \>   $p$ \> \myqed
\end{tabbing}
}
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> \>   $\Event p \lor \neg\Event p$
\end{tabbing}
which is (3.28) Excluded middle, $p\lor\neg p$ with $p := \Event p$. \hfill \myqed\\[\lgap]
...
A similar concept applies to temporal logic theorems in $\mathcal{E}$ except that the implicit application is in
...
$P$ can be proved to be a theorem as follows.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
\> \> $P$\\[\lgap]
...
So, we have $true\impl P$.
By (3.73) Left identity of $\impl$, $true\impl p\equiv p$, we have $P$. \hfill \myqed
...
\emph{Proof:} Suppose $P\impl Q$ is a theorem.
...
Then $\Event P\impl\Event Q$ can be proved to be a theorem as follows.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
...
So, we have $true\impl (\Event P\impl\Event Q)$.
By (3.73) Left identity of $\impl$, $true\impl p\equiv p$, we have $\Event P\impl\Event Q$. \hfill \myqed
...
Then, by (\ref{E:metatheorem}) Metatheorem, $\Always (P\impl Q)$ is a theorem.
Because all theorems are equivalent to each other, and (3.4) $true$ is a theorem, $\Always (P\impl Q)$ is equivalent to $true$.
Then $\Always P\impl\Always Q$ can be proved to be a theorem as follows.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
...
So, we have $true\impl (\Always P\impl\Always Q)$.
By (3.73) Left identity of $\impl$, $true\impl p\equiv p$, we have $\Always P\impl\Always Q$. \hfill \myqed\\
...
\end{tabbing}
which is (\ref{E:distEventAnd}) Distributivity of $\Event$ over $\land$. \hfill \myqed
...
\emph{Proof:} \opt{doc}{ The proof is by (3.61) Contrapositive, $p\impl q\equivs \neg q\impl \neg p$.
Must prove\\
$\neg \Event\Always (p \lor q) \impl \neg (\Event\Always p \lor \Event\Always q)$.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $\neg \Event\Always (p \lor q)$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:dualEvent}) Dual of $\Event$ and (\ref{E:dualAlways}) Dual of $\Always$}\\[\lgap]
  \> \>   $\Always\Event \neg (p \lor q)$\\[\lgap]
  \> $=$  \>  \Hint{(3.47b) De Morgan, $\neg (p \lor q) \equiv \neg p \land \neg q$}\\[\lgap]
  \> \>   $\Always\Event (\neg p \land \neg q)$\\[\lgap]
  \> $\impl$  \>  \Hint{(\ref{E:distAlwaysEventAnd}) Distributivity of $\Always\Event$ over $\land$}\\[\lgap]
  \> \>   $\Always\Event \neg p \land \Always\Event \neg q$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:dualAlways}) Dual of $\Always$ and (\ref{E:dualEvent}) Dual of $\Event$}\\[\lgap]
  \> \>   $\neg\Event\Always p \land \neg\Event\Always q$\\[\lgap]
  \> $=$  \>  \Hint{(3.47b) De Morgan, $\neg (p \lor q) \equiv \neg p \land \neg q$}\\[\lgap]
  \> \>   $\neg (\Event\Always p\lor \Event\Always q)$ \> \myqed
\end{tabbing}
}
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $\impl$  \>  \Hint{(\ref{E:eventAlwaysImp}) $\Event \Always$ implication and (4.3) Monotonicity of $\land$, $(p\impl q)\impl (p\land r \impl q\land r)$}\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$  \>  \Hint{(\ref{E:IdemEvent}) Absorption of $\Event$}\\[\lgap]
  \> \>   $\Event(\Always \neg p \land \Event (p\lor q)) \impl \Event (\neg p \land q)$\\[\lgap]
  \> $\impl$  \>  \Hint{(\ref{E:distEventOr}) Distributivity of $\Event$ over $\lor$}\\[\lgap]
  \> \>   $\Event(\Always \neg p \land \Event (p\lor q)) \impl \Event \neg p \land \Event q$\\[\lgap]
  \> $\impl$  \> \Hint{(3.76b) Strengthening, $p\land q \impl p$} \\[\lgap]
  \> \>   $\Event(\Always \neg p \land \Event (p\lor q)) \impl \Event q$ \> \myqed
\end{tabbing}
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$  \>  \Hint{(3.12) Double negation, $\neg\neg p\equiv p$}\\[\lgap]
  \> \>   $\Event\Always p\land \Event\Always q$ \> \myqed
\end{tabbing}
}
...
Equation (\ref{E:defWait}) is the only defining axiom for the \textit{wait} operator.
...
\end{tabbing}
which is (\ref{E:axiomUntilImpl}) Axiom, $\Until$ implication. \hfill \myqed
...
\> \> $\Always p\lor \Always (p\Until q) \lor p\Until (q\Until r)$\\[\lgap]
\> $\foll$ \> \Hint{(\ref{E:axiomAlwaysUntilImpl}) Axiom, $\Always \Until$ implication}\\[\lgap]
...
(\ref{E:untilOrEquiv}), (\ref{E:untilOrImp}), and (\ref{E:untilAndImp}).
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$ \> \Hint{(3.46) Distributivity of $\land$ over $\lor$, $p\land (q\lor r)\equiv (p\land q)\lor (p\land r)$} \\[\lgap]
  \> \>   $(\Always p\land \Always q) \lor (p \Until r\land q\Until r) \lor (\Always p \land q\Until r) \lor (\Always q\land p \Until r)$\\[\lgap]
  \> $\impl$ \> \Hint{(\ref{E:andUntilDist}) Distributivity of $\land$ over $\Until$, twice and (4.2) Monotonicity of $\lor$, $(p\impl q) \impl (p\lor r \impl q\lor r)$} \\[\lgap]
  ...
  \> $\impl$ \> \Hint{(\ref{E:untilAndImp}) Left distributivity of $\Until$ over $\land$, twice and (4.2) Monotonicity of $\lor$, $(p\impl q) \impl (p\lor r \impl q\lor r)$} \\[\lgap]
  ...
  \> $\impl$ \> \Hint{(3.76b) Strengthening, $p \land q \impl p$ and (4.2) Monotonicity of $\lor$, $(p\impl q) \impl (p\lor r \impl q\lor r)$} \\[\lgap]
  \> \>   $(\Always p\land \Always q) \lor (p \Until r\land q\Until r) \lor (p \Until r\land q\Until r)$\\[\lgap]
  \> $=$ \> \Hint{(3.26) Idempotency of $\lor$, $p\lor p \equiv p$} \\[\lgap]
  \> \>   $(\Always p\land \Always q) \lor (p \Until r\land q\Until r)$\\[\lgap]
  \> $=$ \> \Hint{(\ref{E:untilAndEquiv}) Right distributivity of $\Until$ over $\land$} \\[\lgap]
  \> \>   $(\Always p\land \Always q) \lor (p \land q) \Until r$\\[\lgap]
  \> $\impl$ \> \Hint{(\ref{E:untilImplWait}) $\Until$ in terms of $\Wait$ and (4.2) Monotonicity of $\lor$, $(p\impl q) \impl (p\lor r \impl q\lor r)$} \\[\lgap]
  ...
  \> $=$ \> \Hint{(3.26) Idempotency of $\lor$, $p\lor p \equiv p$} \\[\lgap]
  \> \>   $(p \land q) \Wait r$ \> \myqed
\end{tabbing}



Like the \textit{until} operator, \textit{wait} is idempotent.
$true$ is the right zero of both \textit{until} and \textit{wait}.
$false$ is the left identity of both \textit{until} and \textit{wait}.
The table below summarizes the eight possibilities of $true$ and $false$ on the left hand side and right hand side of $\Until$ and $\Wait$.
...
\> $=$ \> \Hint{(\ref{E:idemUntil}) Idempotency of $\Until$} \\[\lgap]
\> \> $\Always p \lor p$\\[\lgap]
\> $=$ \> \Hint{(\ref{E:absOrIntoAlways}) Absorption of $\lor$ into $\Always$}\\[\lgap]
\> \> $p$ \> \myqed
\end{tabbing}
...
The following theorem for the \textit{wait} operator corresponds to axiom (\ref{E:axiomUntilImpl}) $\Until$ implication.
...
\> $\impl$ \> \Hint{(\ref{E:impAlways}) Strengthening of $\Always$ and (4.2) Monotonicity of $\lor$, $(p\impl q) \impl (p\lor r \impl q\lor r)$} \\[\lgap]
...
\emph{Proof:} By (\ref{E:metatheorem}) Metatheorem and (\ref{E:waitToOr}), $\Always (p\Wait q\impl p\lor q)$ is a theorem.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
  ...
  \> \>   $(p \Wait q) \Wait r \impl (p \lor q) \Wait r$ \> \myqed
\end{tabbing}
