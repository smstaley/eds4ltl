...
% sms begin
To be written.
We extend the development of duality in Gries and Schneider \cite{LADM} section (2.3) to LTL as follows. First, start by modifying definition
(2.2) to include the temporal operators.
...
section (2.3) without change. The proof of this metatheorem is outside the scope of this paper.\\[\lllgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$  \>  \Hint{(3.65) Shunting, $p\land q\impl r\equivs p\impl (q\impl r)$}\\[\lgap]
  \> \>   $\Always p \land \Always q \impl \Always (p \land q)$
\end{tabbing}
And now,
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \end{tabbing}
And now, proof of the first conjunct follows.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> \>   $\Event q$\quad \myqed
\end{tabbing}
And now, proof of the second conjunct follows.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:alwaysImpEvents}) Monotonicity of $\Event$ is a theorem.}\\[\lgap]
  \> \>   $\Always (p \impl q) \impl (\Event p \impl \Event q)$\\[\lgap]
  \end{tabbing}
  By (\ref{E:metaAlways}) Metatheorem $\Always$ the following is a theorem.
  \begin{tabbing}
  \hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $\Always\Always (p \impl q) \impl \Always (\Event p \impl \Event q)$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:IdemAlways}) Absorption of $\Always$}\\[\lgap]
 \> \>   $\Always (p \impl q) \impl \Always (\Event p \impl \Event q)$\\[\lgap]
  \> $\impl$  \>  \Hint{(3.82a) Transitivity, $(p\impl q) \land (q\impl r) \impl (p\impl r)$ 
  and (\ref{E:distAlwaysImp}) Monotonicity of $\Always$ with $p,q := \Event p, \Event q$}\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:distAlwaysImp}) Monotonicity of $\Always$ is a theorem.}\\[\lgap]
  \> \>   $\Always (p \impl q) \impl (\Always p \impl \Always q)$\\[\lgap]
  \end{tabbing}
  By (\ref{E:metaAlways}) Metatheorem $\Always$ the following is a theorem.
  \begin{tabbing}
  \hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
 ...
 \> \>   $\Always (p \impl q) \impl \Always (\Always p \impl \Always q)$\\[\lgap]
  \> $\impl$  \>  \Hint{(3.82a) Transitivity, $(p\impl q) \land (q\impl r) \impl (p\impl r)$ 
  and (\ref{E:alwaysImpEvents}) Monotonicity of $\Event$ with $p,q := \Always p, \Always q$}\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $\impl$  \>  \Hint{(\ref{E:impEvent}) Weakening of $\Event$, with $p := \Always \Event p$}\\[\lgap]
  \> \>   $\Event\Always\Event p$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:absEvent}) Absorption of $\Event$ into $\Always\Event$}\\[\lgap]
  \> \>   $\Always\Event p$
\end{tabbing}
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $\impl$  \>  \Hint{(\ref{E:impEvent}) Weakening of $\Event$, with $p :=  \Event\Always p$}\\[\lgap]
...
In the text by Ben-Ari \cite{Ben} the dual of the binary operator $p \Until q$ is defined as $\neg(\neg p \Until \neg q)$ 
and called the Release operator. To avoid adding another operator to our system, we use the following four theorems
