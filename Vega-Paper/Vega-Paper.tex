...
\emph{Proof}: Theorem (\ref{E:rightMonoUntil}) Right monotonicity of $\Until$ with $p,q,r := q \land r, q\Until r,p$ is
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
\> \> $\Always (q \land r \impl q\Until r) \impl (p\Until (q\land r)\impl p\Until (q\Until r))$
\end{tabbing}
Because the consequent is the theorem to be proved, it suffices to prove the truth of the antecedent.
By (\ref{E:metatheorem}) Metatheorem, the antecedent is valid iff
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
\> \> $q \land r \impl q\Until r$
\end{tabbing}
is valid.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
