...
Next $\Next$ theorem using metatheorem duality.

Proof of (\ref{E:distNextAnd}) Distributivity of $\Next$ over $\land$:$\quad \Next (p \land q) \equiv \Next p \land \Next q$
using duality.
...
This two-step equational proof of (\ref{E:distNextAnd}) can be compared with the seven-step equational proof
 ...
 Until $\Until$ theorem using metatheorem duality.
...
Eventually $\Event$ theorem using metatheorem duality.

Always $\Always$ theorem using metatheorem duality.

Wait $\Wait$ theorem using metatheorem duality.
...
% sms begin
% This is Ben-Ari Axiom 3 on page 263
...
\end{equation}
\emph{Proof}: The proof is by Truth equivalence.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:impAlwaysNA}) Strengthening of $\Always$ is a theorem}\\[\lgap]
  \> \>   $\Always p \impl \Next \Always p$\\[\lgap]
   \> $=$  \>  \Hint{(\ref{E:dNextAlways}) Exchange of $\Next$ and $\Always$}\\[\lgap]
   \> \>   $\Always p \impl \Always\Next p$ \quad \myqed
\end{tabbing}
% sms end
...
%
% sms - Frame Laws Here (except for Wait Laws).
%
...
come from Kröger and Merz\cite{Kroger} and say that if $\Always p$ holds then $p$ may be "added"
(by conjunction) under each temporal operator.
...
\end{tabbing}
The frame law of theorem (\ref{E:framelawEvent}) is equivalent to theorem (\ref{E:alwaysAndEvent})
...
\end{tabbing}
In theorems (\ref{E:disjframelawnext}) to (\ref{E:untilframelawAlways}) the frame laws are proven
to extend to all binary propositional and temporal operators.
