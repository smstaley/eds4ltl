...
The axiom (\ref{E:distEventAlwaysAndAx}) is required to prove the distributivity of $\Event\Always$ over $\land$.
...
\textbf{Axiom:}\quad \Event\Always p \land \Event\Always q \impl \Event\Always(p \land q)
\end{equation}
...
\end{tabbing}
which is (\ref{E:distAlwaysOr}) Distributivity of $\Always$ over $\lor$. \hfill \myqed
...
Like the \textit{until} operator, the \textit{wait} operator is partially associative.
...
\> $\foll$ \>  \Hint{(\ref{E:untilAssocImp}) Partial associativity of $\Until$}\\[\lgap]
\> \> $\Always p\lor \Always (p\Until q) \lor p\Until (q\Until r)$\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $\impl$  \>  \Hint{(\ref{E:waitAssocImp}) Partial associativity of $\Wait$}\\[\lgap]
  \> \>   $(p \Wait p) \Wait q$\\[\lgap]
