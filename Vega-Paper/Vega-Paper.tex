...
The distributivity of \textit{next} over \textit{until} (\ref{E:distNextUntil}) implies the distributivity of \textit{next} over \textit{wait} as Section (3.7) shows.
...
\emph{Proof}: (Scott Staley) The proof is by (4.7.1) Truth implication.
\opt{doc}{
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:andUntilDist}) Distributivity of $\land$ over $\Until$ with $q,r :=true, q$}\\[\lgap]
  \> \>   $\Always p \land true \Until q \impl (p \land true) \Until (p \land q)$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:untilAndEquiv}) Right distributivity of $\Until$ over $\land$}\\[\lgap]
  \> \>   $\Always p \land true \Until q \impl p\Until (p\land q) \land true \Until (p \land q)$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:defEvent}) Definition of $\Event$, twice}\\[\lgap]
  \> \>   $\Always p \land \Event q \impl p\Until (p\land q) \land \Event (p \land q)$\\[\lgap]
  \> $\impl$  \>  \Hintfirst{(3.76b) Strengthening the antecedent $p\land q\impl p$, and (3.82a) Transitivity}\\[\lgap]
  \> \>  \Hintlast{$(p\impl q)\land (q\impl r)\impl (p\impl r)$}\\[\lgap]
  \> \>   $\Always p \land \Event q \impl \Event (p \land q)$ \quad \myqed
\end{tabbing}
}
\opt{student}{
Exercise for the student.\\
...
Theorems (\ref{E:contradiction}) and (\ref{E:contradictionb}) are the linear temporal versions of the contraction theorem of propositional logic, (3.42) $p\land\neg p\equiv false$.
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$  \> \Hint{(\ref{E:eventImpAlways}) Distributivity of $\Event$ over $\impl$}\quad [\textit{Error}]\\[\lgap]
...
Both $\Until$ and $\Wait$ obey the following disjunction rules.
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
  \> $=$ \> \Hint{(\ref{E:untilImplWait}) with $p,q := \neg q, p\land \neg q$}\\[\lgap]
  \> \>   $\neg q\Until (p\land\neg q)\impl \neg q\Wait(p\land \neg q)$\\[\lgap]
  \> $\impl$  \>  \Hintfirst{Lemma: $\neg q\Wait(p\land\neg q)\impl\neg q\Wait p$ and (3.82a) Transitivity,}\\[\lgap]
  \> \> \Hintlast{$(p\impl q)\land (q\impl r)\impl (p\impl r)$} \\[\lgap]
  \> \>   $\neg q\Until (p\land\neg q)\impl \neg q\Wait p$\\[\lgap]
  \> $=$  \>  \Hint{(3.59) Implication, $p\impl q\equivs\neg p \lor q$}\\[\lgap]
  \> \>   $\neg(\neg q\Until (p\land\neg q))\lor \neg q\Wait p$\\[\lgap]
  ...
  \> \>   $\neg (p\land \neg q)\Wait (q\land \neg(p\land\neg q))\lor \neg q\Wait p$\\[\lgap]
  \> $=$  \>  \Hint{(3.47a) De Morgan, $\neg(p\land q)\equivs \neg p\lor\neg q$ twice}\\[\lgap]
  \> \>   $(\neg p\lor q)\Wait (q\land (\neg p\lor q))\lor \neg q\Wait p$\\[\lgap]
  \> $=$  \>  \Hint{(3.43a) Absorption, $p\land (p\lor q)\equiv p$}\\[\lgap]
  \> \>   $(\neg p\lor q)\Wait q\lor \neg q\Wait p$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:disjunctWait}) Disjunction rule of $\Wait$}\\[\lgap]
  \> \>   $\neg p\Wait q\lor \neg q\Wait p$
\end{tabbing}
\emph{Proof of Lemma}: The proof is by (4.7.1) Truth implication.
(3.76b) Strengthening the antecedent $p\land q\impl p$ with $q := \neg q$ is $p\land \neg q\impl p$.
Therefore, by (\ref{E:metatheorem}) Metatheorem, $\Always (p\land \neg q\impl p)$ is a theorem.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $true$\\[\lgap]
  \> $=$  \>  \Hint{$\Always (p\land \neg q\impl p)$ is a theorem}\\[\lgap]
  \> \>   $\Always (p\land \neg q\impl p)$\\[\lgap]
  \> $\impl$ \> \Hint{(\ref{E:rightMonoWait}) Right monotonicity of $\Wait$ with $p,q,r:=p\land\neg q,p,\neg q$} \\[\lgap]
  \> \>   $\neg q\Wait(p\land\neg q)\impl\neg q\Wait p$ \quad \myqed
\end{tabbing}
