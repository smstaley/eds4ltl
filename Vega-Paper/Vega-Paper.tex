...
\> $=$ \> \Hint{(\ref{E:distNextOr})} \\[\lgap]
\> \> $\Next(p\Until q)\lor \Next \Always p$\\[\lgap]
\> $=$ \> \Hint{(\ref{E:distNextUntil}) and (\ref{E:dNextAlways})} \\[\lgap]
...
\> $\impl$ \> \Hint{(\ref{E:distAlwaysOr}) and (4.2) Monotonicity of $\lor$, $(p\impl q)\impl (p\lor r \impl q\lor r)$} \\[\lgap]
\> \> $(p \Until r) \lor (q \Until r) \lor \Always (p \lor q)$\\[\lgap]
\> $\impl$ \> \Hint{(\ref{E:untilOrImp}) and (4.2) Monotonicity of $\lor$, $(p\impl q)\impl (p\lor r \impl q\lor r)$} \\[\lgap]
