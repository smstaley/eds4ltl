...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $\impl$  \>  \Hint{(\ref{E:alwaysImpEvents}) Monotonicity of $\Event$ and (\ref{E:monoAlwaysEvent}) Monotonicity of $\Always\Event$}\\[\lgap]
  \> \>   $\Event(\Always\Event p\impl \Always\Event\Always q)$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:absAlways}) Absorption of $\Always$ into $\Event\Always$}\\[\lgap]
