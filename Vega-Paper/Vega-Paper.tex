...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
\> \> $true$\\[\lgap]
  \> $\impl$ \> \Hint{(\ref{E:UntilOrdering}) $\Until$ ordering with $q,r:=p,q$} \\[\lgap]
\> \> $p\Until p \land \neg p\Until q \impl p\Until q$\\[\lgap]
\> $=$ \> \Hint{(\ref{E:idemUntil}) Idempotency of $\Until$} \\[\lgap]
\> \> $p \land \neg p\Until q \impl p\Until q$ \quad \myqed
\end{tabbing}
...
\> $=$ \> \Hint{(3.30) Identity of $\lor$, $p\lor false\equiv p$} \\[\lgap]
  ...
  \> $\impl$  \>  \Hint{(\ref{E:UntilOrdering}) $\Until$ ordering and (4.2) Monotonicity of $\lor$}\\[\lgap]
