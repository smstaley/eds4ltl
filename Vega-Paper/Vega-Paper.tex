...
\begin{tabular}{c|cccccccccccc}
  $\sigma$                  & $s_0$ & $s_1$ & $s_2$ & $s_3$ & $s_4$ & $s_5$ & $s_6$ & $s_7$ & $s_8$ & \dots \\
  \hline
  $p$                       & F     & T     & F     & T     & F     & T     & T     & T     & F     &  \dots\\
  $q$                       & F     & F     & T     & F     & T     & T     & F     & F     & F     &  \dots\\
  $r$                       & F     & F     & F     & F     & T     & T     & T     & F     & T     &  \dots\\
  $p\Until q$               & F     & T     & T     & T     & T     & T     & F     & F     & F     &  \dots\\
  $q\Until r$               & F     & F     & F     & F     & T     & T     & T     & F     & T     &  \dots\\
  $p\Until (q\Until r)$     & F     & F     & F     & T     & T     & T     & T     & T     & T     &  \dots\\
  $(p\Until q)\Until r$     & F     & T     & T     & T     & T     & T     & T     & F     & T     &  \dots
\end{tabular}\\[\lllgap]
States $s_1$ and $s_2$ in the last two rows of the above table show that $(p\Until q)\Until r$ does not imply $p\Until (q\Until r)$, and state $s_7$ shows that $p\Until (q\Until r)$ does not imply $(p\Until q)\Until r$.
...
\centering
  ...
  \put(28,168){(\ref{E:untilImplWait})}
  \put(128,168){(\ref{E:RightWaitUntilImpl})}
  \put(280,168){(\ref{E:untilImplWait})}
  \put(380,168){(\ref{E:LeftUntilWaitImpl})}
  \put(22,98){(\ref{E:RightUntilUntilImpl})}
  \put(128,98){(\ref{E:untilImplWait})}
  \put(272,98){(\ref{E:LeftUntilUntilImpl})}
  \put(376,98){(\ref{E:untilImplWait})}
  \put(168,48){(\ref{ })}
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> \>   $p\Wait r$ \quad \myqed
\end{tabbing}
Lemma: $\Always ((p \lor q) \land (\neg q \lor r))\impl \Always(p\lor r)$

\emph{Proof}: By (\ref{E:metaAlways}) Metatheorem $\Always$, if
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
\> \> $(p\lor q)\land (\neg q\lor r) \impl p\lor r$
\end{tabbing}
is valid, then the Lemma is valid.
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $(p\lor q)\land (\neg q\lor r)$\\[\lgap]
  \> $=$  \>  \Hint{(3.46) Distributivity of $\land$ over $\lor$, $p\land(q\lor r) \equiv (p\land q)\lor (p\land r)$}\\[\lgap]
