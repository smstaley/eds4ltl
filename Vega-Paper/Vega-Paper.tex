...
\newcommand{\myqedtab}{\hspace{384pt}}              % For flush right qed symbol in tabbing environment
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> \>   $p \Until q \land \Event q$\\[\lgap]
  \> $\impl$  \>  \Hint{(3.76b) Strengthening the antecedent}\\[\lgap]
  \> \>   $\Event q$ \> \myqed
\end{tabbing}
...
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $\Event\Always p \land (\Always p \impl \Event q)$\\[\lgap]
  \> $\impl$  \>  \Hint{(\ref{E:impEvent}) Weakening of $\Event$ and (4.3) Monotonicity of $\land$, $(p\impl q)\impl (p\land r\impl q\land r)$}\\[\lgap]
