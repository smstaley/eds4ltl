...
\textbf{NOTE:} Need commentary and possible name for this theorem.
\begin{equation}\label{E:eventPImplEventQ}
(\Event p \impl \Event q) \impl \Event (p \impl q)
\end{equation}

\emph{Proof}: (Alex Angelo)
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $\Event (p \impl q)$\\[\lgap]
  \> $=$  \>  \Hint{(3.59) Implication, $p\impl q \equivs \neg p \lor q$}\\[\lgap]
...
Both $\Wait$ and $\Until$ obey the following disjunction rules.
