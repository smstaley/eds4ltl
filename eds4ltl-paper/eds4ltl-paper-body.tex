...
\begin{figure}[t]
\centering
\begin{picture}(208,88)
\thicklines
\put(0,80) {\fbox{\parbox{36pt}{\centering Input}}}
\put(43,82) {\vector(1,0){20}}
\put(63,80) {\fbox{\parbox{56pt}{\centering Processing}}}
\put(126,82) {\vector(1,0){20}}
\put(146,80) {\fbox{\parbox{42pt}{\centering Output}}}
\put(40,60) {Computational systems}

\put(0,24) {\fbox{\parbox{36pt}{\centering Axioms}}}
\put(43,26) {\vector(1,0){20}}
\put(63,24) {\fbox{\parbox{56pt}{\centering Inference\\rules}}}
\put(126,26) {\vector(1,0){20}}
\put(146,24) {\fbox{\parbox{48pt}{\centering Theorems}}}
\put(40,0) {Axiomatic logic systems}
...
The propositional calculus theorems from LADM are included in the Supplementary Materials section of the online version of this paper.
...
In the equational logic of LADM, \textit{true} is theorem (3.4) and is equivalent to all other theorems.
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$  \>  \Hint{(\ref{E:linearity}) Linearity}\\[\lgap]
  \> \>   $\Next \mathit{true} \equiv \mathit{true}$\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$  \>  \Hint{(3.57) Definition of implication, $p\impl q\equivs p\lor q \equivs q$}\\[\lgap]
  \> \>   $p \land q \impl p \Until q$\\[\lgap]
   \> which is (\ref{E:andImplUntil}). \quad \myqed
\end{tabbing}
}
...
The two induction axioms are unique to this system.
...
The induction theorem (\ref{E:induction}) is common to many systems.
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> which is (\ref{E:induction}) $\Always$ Induction. \quad \myqed
\end{tabbing}
}
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> \>   $\Event \mathit{true} \equiv \mathit{true}$\\[\lgap]
...
These theorems are obvious, and their proofs are simple, which is perhaps why they do not appear in other systems.
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$  \>  \Hint{(\ref{E:dualEvent}) Dual of $\Event$}\\[\lgap]
  \> \>   $\Event p \land \neg\Event p \equivs \mathit{false}$\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $\Always p \land \Event\neg p \equivs \mathit{false}$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:dualAlways}) Dual of $\Always$}\\[\lgap]
  \> \>   $\Always p \land \neg\Always p \equivs \mathit{false}$\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
 ...
 \> $=$  \>  \Hint{(\ref{E:dualAlwaysEvent}) Dual of $\Always \Event$}\\[\lgap]
  \> \>   $\Always \Event p \land \neg \Always \Event p \equivs \mathit{false}$\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
 ...
 \> $=$  \>  \Hint{(\ref{E:dualEventAlways}) Dual of $\Event \Always$}\\[\lgap]
  \> \>   $\Event \Always p \land \neg \Event \Always p \equivs \mathit{false}$\\[\lgap]
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
	...
	\> $=$  \>  \Hint{(\ref{E:nextTruth}) Truth of $\Next$}\\[\lgap]
	\> \>   $\Always p \impl \mathit{true}$\\[\lgap]
	\> which is (3.72) Right zero of $\impl$, $p\impl \mathit{true} \equivs \mathit{true}$ \quad \myqed
\end{tabbing}
}
...
The conjunction and disjunction rules are unique to this system.
...
\begin{printonly}
The appendix available in the online version is a collection of the axioms and theorems of the propositional calculus in Gries and Schneider's book \textit{A Logical Approach to Discrete Math}.\cite{LADM}
\end{printonly}

\begin{screenonly}
This section is a collection of the axioms and theorems of the propositional calculus in Gries and Schneider's book \textit{A Logical Approach to Discrete Math}.\cite{LADM}
...
Research supported the second author by Tooma Undergraduate Research Fellowship Program, Pepperdine University, Summer 2009 and academic year 2009-10.

\end{acks}

% Bibliography
\bibliographystyle{ACM-Reference-Format}
\bibliography{eds4ltl-bibliography}
